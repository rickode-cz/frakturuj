<script>
 ...
 async function handleICOLookup() {
  if (!$profile.ico || $profile.ico.length !== 8) {
  ...
  try {
   loading = true;
   error = null;
   const data = await prefillByICO($profile.ico);
 ...
 async function handleSubmit() {
  try {
   ...
   if (response.status === "success") {
    // Update the profile in apiData store
    apiData.update((data) => ({
     ...data,
   ...
   notification.set({
    type: "error",
    message: "Nepodařilo se uložit profil. Zkuste to prosím znovu.",
   });
  } finally {
   saving = false;
  }
...
>
 <div class="p-6">
  <div class="flex justify-between items-center mb-6">
   <h2 class="text-2xl">Můj profil</h2>
   <button
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-4">
   <div class="flex gap-4">
    <div class="flex-grow">
     <label for="ico" class="block text-base text-zinc-400 mb-1">IČO</label>
     <input
      id="ico"
      type="text"
      bind:value={$profile.ico}
   ...
   <div class="grid grid-cols-2 gap-4">
    <div>
     ...
     <input
      id="bank_account"
      type="text"
      bind:value={$profile.bank_account}
      class="w-full bg-zinc-700 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
      required
     />
    </div>
    <div>
     <label for="bank_code" class="block text-base text-zinc-400 mb-1"
      >Kód banky</label
     >
     <input
      id="bank_code"
      type="text"
      bind:value={$profile.bank_code}
      class="w-full bg-zinc-700 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
      required
     />
    </div>
