<script>
    ...
    async function handleDelete() {
        try {
            ...
            apiData.update(data => ({
                ...data,
                invoices: data.invoices.filter(inv => inv.id !== invoice.id)
            }));
            ...
            notification.set({
                type: 'error',
                message: e.message || 'Nepodařilo se smazat fakturu'
            });
        } finally {
            deleting = false;
        }
...
<div class="fixed inset-4 md:inset-x-1/3 md:inset-y-1/3 bg-zinc-800 rounded-lg shadow-xl z-[999]">
    <div class="p-6 h-full flex flex-col">
        <div class="flex justify-between items-start">
            <h2 class="text-2xl">Smazat fakturu</h2>
            <button class="cursor-pointer text-zinc-400 hover:text-white" on:click={onClose}>
        ...
        <div class="my-6 text-zinc-300">
            <p>Opravdu chcete smazat fakturu <strong>{invoice.title}</strong>?</p>
            <p class="mt-2 text-red-400">Tuto akci nelze vrátit zpět!</p>
        </div>
